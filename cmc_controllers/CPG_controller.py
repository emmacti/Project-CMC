...
class CPGNetwork(NeuralNetwork):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Q3.1: stretch feedback weight (optional)
        self.w_ipsi = kwargs.pop('w_ipsi', None)

        # Q3.3: disruption parameters – stored before building the masks below
        ...
        if self.disruption_p_sensors > 0.0:
            sensor_mask_vals = rng.uniform(0.0, 1.0, size=self.n_body_joints)
            self.sensor_disruption_mask = sensor_mask_vals < self.disruption_p_sensors
        else:
        ...
        if self.disruption_p_couplings > 0.0:
            # Build mask covering only the ipsilateral connections.
            ipsi_only = np.zeros(
                (self.n_oscillators, self.n_oscillators), dtype=bool)
            for k in range(self.n_body_joints - 1):
            ...
            coupling_vals = rng.uniform(
                0.0, 1.0, size=(self.n_oscillators, self.n_oscillators))
            disrupted_couplings = (coupling_vals < self.disruption_p_couplings) & ipsi_only
            # Zero out disrupted weights permanently
            self.coupling_weights[disrupted_couplings] = 0.0
        # (if p_couplings == 0, coupling_weights are left exactly as built above)
    ...
    def network_ode(self, _time, state, stretch_value):
        ...
        state: [phases, amplitudes, motor_outputs_storage]
        stretch_value: array of mechanical joint angles (n_body_joints,)
        Returns: derivatives for [phases, amplitudes, zeros_for_storage]
        """
        ...
        if self.w_ipsi is None or float(self.w_ipsi) == 0.0:
            ...
            if len(stretch_value) != self.n_body_joints:
                raise ValueError(
                    f"stretch_value must have length n_body_joints={self.n_body_joints}, "
                    f"got {len(stretch_value)}"
                )

            # Eq. v(theta): left = max(0, theta), right = max(0, -theta)
            s_left  = self.w_ipsi * np.maximum(0.0,  stretch_value)
            s_right = self.w_ipsi * np.maximum(0.0, -stretch_value)

            # Q3.3: zero out muted sensors (mask is per body joint)
...
class CPGController(PolymanderController):
    ...
    def __init__(self,
        ...
        animat_network = CPGNetwork(
            ...
            w_ipsi=config.get('w_ipsi', None),
            disruption_p_sensors=config.get('disruption_p_sensors', 0.0),
            disruption_p_couplings=config.get('disruption_p_couplings', 0.0),
