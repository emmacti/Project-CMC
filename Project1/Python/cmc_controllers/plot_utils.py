...
def load_sim(hdf5_path):
    """Load and align all arrays to the same length."""
    ...
    n_sensor = links_positions.shape[0]

    # Build a perfectly uniform time axis from the sensor array length.
    # Use the timestep inferred from the HDF5 times array.
    # The HDF5 times axis may be longer than sensor arrays (different save rates).
    t0 = float(times_raw[0])
    # Find dt robustly: use the most common diff in times_raw
    raw_diffs = np.diff(times_raw[:min(50, len(times_raw))])
    dt = float(np.median(raw_diffs))
    times = t0 + np.arange(n_sensor) * dt

    return (times, links_positions, links_velocities,
            joints_positions, joints_velocities, joints_torques)


def load_controller(pkl_path, n_sensor=None):
    """Load controller pickle, optionally sub-sample to sensor length."""
    ...
    if n_sensor is not None and state.shape[0] != n_sensor:
        # Sub-sample uniformly — take every k-th row to match sensor length
        step  = state.shape[0] / n_sensor
        idx   = np.round(np.arange(n_sensor) * step).astype(int)
        idx   = np.clip(idx, 0, state.shape[0] - 1)
        state = state[idx]
...
def analysis_3_1():
    ...
    for label, cfg in cases.items():
        ...
        state, n_osc, n_body = load_controller(cfg['pkl'], n_sensor=len(times))
...
def analysis_3_2():
    ...
    for w in w_vals_found:
        ...
        state, n_osc, n_body = load_controller(pkl_p, n_sensor=len(times))
...
def print_comparison_table():
    ...
    def _load_metrics(hdf5_p, pkl_p):
        if not os.path.isfile(hdf5_p) or not os.path.isfile(pkl_p):
            return None
        (times, lp, lv, jp, jv, jt) = load_sim(hdf5_p)
        state, n_osc, _ = load_controller(pkl_p, n_sensor=len(times))
        _, _, ml, mr    = unpack_controller(state, n_osc)
